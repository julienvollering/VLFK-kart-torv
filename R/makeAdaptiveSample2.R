...
sadf <- read_csv(file="output/samplingMatrix.csv") |> 
  filter(road == 0, rwalkcost < 8000, canopy < 50)
...
# Select 25 highest priority localities ####
...
localities <- df |>
  ...
  st_write('data/GIS/samplingDesign.gpkg', layer = 'adaptivesample2-localites', append=FALSE)

# Subset population further by rwalkcost ####

reach <- rast("output/rwalk-cost-adaptivesample2.tif")
...
sample <- samplingpop |>
  mutate(wgt = range01(pred)*range01(pred.se), .after = pred.se) |> 
  slice_sample(n = 50, weight_by = wgt) |> 
  st_as_sf(coords = c('x','y'), crs="EPSG:25833") |> 
  select(pred, DTM, slope, TPI, MRVBF, continentality) |> 
  st_write('data/GIS/samplingDesign.gpkg', layer = 'adaptivesample2', append=FALSE)
