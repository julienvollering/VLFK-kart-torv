...
localities <- df |>
  filter(rwalkcost < 1000) |> 
  mutate(wgt = range01(pred)*range01(pred.se), .after = pred.se) |> 
  arrange(desc(wgt)) |> 
  slice_head(n = 25) |> 
  st_as_sf(coords = c('x','y'), crs="EPSG:25833") |> 
  select(pred, pred.se, wgt) |> 
  st_write('data/GIS/samplingDesign.gpkg', layer = 'adaptivesample3-localites', append=FALSE)

# Subset population further by rwalkcost ####

reach <- rast("output/rwalk-cost-adaptivesample3.tif")
...
sample <- samplingpop |>
  mutate(wgt = range01(pred)*range01(pred.se), .after = pred.se) |> 
  slice_sample(n = 60, weight_by = wgt) |> 
  st_as_sf(coords = c('x','y'), crs="EPSG:25833") |> 
  select(pred, DTM, slope, TPI, MRVBF, continentality) |> 
  st_write('data/GIS/samplingDesign.gpkg', layer = 'adaptivesample3', append=FALSE)
